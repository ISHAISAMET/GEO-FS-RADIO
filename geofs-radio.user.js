...
(function () {
  ...
  function loadSettings() {
    try {
      ...
      if (raw) {
        ...
        // Add radio 3 automatically for users who already have
        // an older saved configuration with only two radios.
        if (!settings.radios) settings.radios = [];

        if (!settings.radios.some(r => r.id === 3)) {
        ...
        if (!settings.joystickBindings) settings.joystickBindings = {};
        if (settings.panelVisible === undefined) settings.panelVisible = true;

        return settings;
      }
    } catch (e) {}

    // Three radios by default. All can be tuned to either band.
    return {
      radios: [
        { id: 1, frequency: DEFAULT_FREQUENCY, mode: 1, power: false },
        { id: 2, frequency: DEFAULT_FREQUENCY, mode: 1, power: false },
        { id: 3, frequency: DEFAULT_FREQUENCY, mode: 1, power: false }
      ],
      keyBindings: {},
      joystickBindings: {},
      panelVisible: true
  ...
  let listeningForJoystickBind = null;
  let micStream = null;

  // deviceId -> { peerId: RTCPeerConnection }
  const peerConnectionsByDevice = {};
  state.radios.forEach(r => {
    peerConnectionsByDevice[r.id] = {};
  });
  ...
  function stepFrequency(radio, direction) {
    const band =
      BANDS.find(
        b => radio.frequency >= b.min - 1e-9 &&
             radio.frequency <= b.max + 1e-9
      ) || BANDS[0];
  ...
  function render() {
    ...
    state.radios.forEach(radio => {
      ...
      unit.innerHTML = `
        <button class="geofs-gear-btn"
                data-action="settingsToggle"
                title="Settings">&#9881;</button>

        <div class="geofs-settings-menu" data-menu>
          <button data-action="bindKey">
            Key: ${state.keyBindings[radio.id] || '-'}
          </button>

          <button data-action="bindJoystick">
            Joystick: ${
              state.joystickBindings[radio.id] !== undefined
                ? ('Button ' + state.joystickBindings[radio.id])
                : '-'
            }
          </button>
        </div>

        <div class="geofs-radio-screen" data-action="freqClick">
          ${radio.frequency.toFixed(2)}
        </div>
        ...
        <div class="geofs-radio-row">
          <button data-action="power"
                  class="geofs-power-btn ${radio.power ? 'on' : 'off'}">
            ${radio.power ? 'ON' : 'OFF'}
          </button>
        </div>
      ...
      unit.querySelector('[data-action="freqClick"]').onclick = () => {
        ...
        if (!isNaN(val) && clampFrequency(val)) {
          ...
          alert(
            'Invalid frequency. Must be within 108.00-135.90 or 1100.00-3900.90'
          );
        }
      };

      unit.querySelector('[data-action="down"]').onclick = () => {
        stepFrequency(radio, -1);
      };

      unit.querySelector('[data-action="up"]').onclick = () => {
        stepFrequency(radio, 1);
      };

      unit.querySelector('[data-action="mode"]').onclick = () => {
        cycleMode(radio);
      };

      unit.querySelector('[data-action="power"]').onclick = () => {
        togglePower(radio);
      };
      ...
      unit.querySelector('[data-action="bindJoystick"]').onclick = () => {
        ...
        unit.querySelector(
          '[data-action="bindJoystick"]'
        ).textContent = 'Press a joystick button...';
      };
  ...
  function setPttDot(deviceId, active) {
    ...
    const idx = state.radios.findIndex(
      r => r.id === deviceId
    );
  ...
  window.addEventListener('keydown', e => {
    ...
    const devId = Object.keys(state.keyBindings).find(
      id => state.keyBindings[id] === e.code
    );
  ...
  function pollGamepad() {
    ...
    if (gp) {
      if (listeningForJoystickBind) {
        const pressedIndex = gp.buttons.findIndex(
          b => b.pressed
        );

        if (pressedIndex !== -1) {
          state.joystickBindings[listeningForJoystickBind] =
            pressedIndex;
        ...
        Object.entries(state.joystickBindings).forEach(
          ([deviceId, buttonIndex]) => {
            const pressed =
              !!gp.buttons[buttonIndex]?.pressed;
  ...
  function setPTT(deviceId, pressed) {
    const radio = state.radios.find(
      r => r.id === deviceId
    );

    if (!radio || !radio.power) return;

    const clone = micClonesByDevice[deviceId];

    if (clone) {
      clone.enabled = pressed;
    }
  ...
  async function initMic() {
    try {
      ...
      console.error(
        'GeoFS Radio: could not access the microphone',
        e
      );
    }
  }
  ...
  function getMicCloneForDevice(deviceId) {
    if (!micClonesByDevice[deviceId] && micStream) {
      const track =
        micStream.getAudioTracks()[0].clone();
  ...
  function handleServerMessage(msg) {
    ...
    if (msg.type === 'peers') {
      msg.peers.forEach(peerId => {
        createOffer(msg.deviceId, peerId);
      });
    }

    if (msg.type === 'peer-left') {
      Object.keys(peerConnectionsByDevice).forEach(
        devId => {
          const pc =
            peerConnectionsByDevice[devId][msg.id];

          if (pc) {
            pc.close();
            delete peerConnectionsByDevice[devId][msg.id];
          }
        }
      );
    }
  ...
  function rejoinChannel(radio) {
    ...
    Object.keys(
      peerConnectionsByDevice[radio.id]
    ).forEach(peerId => {
      peerConnectionsByDevice[radio.id][peerId].close();

      delete peerConnectionsByDevice[
        radio.id
      ][peerId];
    });
